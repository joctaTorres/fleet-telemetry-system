...
description: Propose a batch: a guided, anti-waterfall flow that slices an objective into ordered vertical-slice phases with per-phase proofs-of-work and writes a batch manifest (not change directories). Use when the user wants to plan a phased multi-change effort.
license: MIT
...
Propose a batch — a phased, anti-waterfall unit of work — by guiding the
